...
var margins = function(){
    if($('.big').length > 0){
        ...
        if (textHeight > imgDivHeight) {
            ...
            if ($(window).width() <= 768) {
                rowHeight = imgDivHeight + 20;
                $('.big').children('.myhide').css({'margin-top': '10px', 'margin-bottom': '10px'})
            }
        };
    };
};
...
original div that the user clicked on.*/
    ...
    var big = function(target, animate){ //animate is a true/false var to set animation or not
        ...
        $(rowDiv).find('.hideClose').css("display", "block")
        $(rowDiv).css('backgroundColor','#EEEEEE')
        $(rowDiv).css('marginBottom','5px')
        $(userProfilePic).find("h4").remove()
        ...
        $(userProfileImg).height(500);
        $(userProfileInfo).css({'display':'block',"fontSize":"1.1em"});
        if (userProfileInfo.length===0) {
            $(userProfilePic).removeClass("col-sm-3 col-xs-6").addClass("col-xs-12 col-md-4 col-md-offset-4 col-xs-offset-3")
        }
        else{
            $(userProfilePic).removeClass("col-sm-3 col-xs-6").addClass("col-xs-12 col-md-8 col-md-offset-0")
        };
